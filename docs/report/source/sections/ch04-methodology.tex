...
Table~\ref{tab:arch-compare} contrasts the decommissioned LiveKit pipeline with the deployed Gemini Proxy path along dimensions that motivated the migration.

\tablesetup
\begin{table}[H]
\centering
\caption{Architectural comparison: legacy LiveKit pipeline vs.\ Gemini Proxy (production, May 2026).}
\label{tab:arch-compare}
\small
\begin{tabular}{|p{3.0cm}|p{5.1cm}|p{5.1cm}|}
\hline
\TblHead{Dimension} & \TblHead{Legacy (LiveKit + Python agent)} & \TblHead{Current (Gemini Proxy)} \\
\hline
Audio path hops & Browser $\rightarrow$ LiveKit room $\rightarrow$ agent $\rightarrow$ STT $\rightarrow$ LLM $\rightarrow$ TTS $\rightarrow$ room $\rightarrow$ browser & Browser $\rightarrow$ proxy $\rightarrow$ Gemini Live $\rightarrow$ browser \\
Auth enforcement & Room token; model unaware of user identity & JWT verified before upstream WSS opens \\
Modalities & Voice-first; screen/webcam via plugins & Native multimodal stream in one session \\
Session cost visibility & Opaque plugin + room minutes & \texttt{usageMetadata} $\rightarrow$ \texttt{gemini\_sessions} \\
Typical p50 round-trip & 650--900\,ms (staging replay) & 380--520\,ms (staging replay) \\
Concurrent sessions / 2\,vCPU & 120--180 & 350--520 \\
\hline
\end{tabular}
\end{table}
